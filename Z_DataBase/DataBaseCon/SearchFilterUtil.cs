...
namespace Z_DataBase.DataBaseCon
{
    ...
    public static class SearchFilterUtil
    {
        ...
        /// <returns>返回的实体信息</returns>
        public static T ConvertCondition<T>(string condition)
        {
            var mstream = new MemoryStream(Encoding.UTF8.GetBytes(condition));
            var dcjson = new DataContractJsonSerializer(typeof(T));
            return (T)dcjson.ReadObject(mstream);
        }
    }
